          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col  cols="12"   >
                    <v-text-field v-model="editedItem.message.messageTitle" label="名称" ></v-text-field>
                  </v-col>
                  <v-col cols="12"   >
                    <v-text-field v-model="editedItem.content" label="描述"  ></v-text-field>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
...
<script>
  export default {
    ...
    data: () => ({
      ...
      dialogDelete: false,
      headers: [
        { text: '名称', value: 'message.messageTitle' },
    ...
    methods: {
      ...
      deleteItemConfirm () {
       this.axios.post('/comment/delete',this.deleteItemObj).then((res)=>{
			const{data} = res;
			if(data.success){
				this.getareaList();
			}
      ...
      save () {
        this.axios.post('/comment/save',this.editedItem).then((res)=>{
			const{data} = res;
			if(data.success){
				this.getareaList();
			}
      ...
      getareaList(){
		this.axios.post('/comment/list',this.searchItem).then((res)=>{
			const{data} = res;
			if(data.success){
